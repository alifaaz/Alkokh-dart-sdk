...
Added `listPetBreeds()`, backed by `pet_app.api.mobile.pets.list_breeds`, so pet forms can render the backend `Pet Breed` master with optional animal type/species/search filters.
...
No breaking changes. 46 tests passing, `dart analyze` clean.
